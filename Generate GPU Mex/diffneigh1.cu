...
__global__ void cudaLaunch(float *imData,float *output,int nSize,int searchSize,mwSize dx,mwSize dy,mwSize dz){
	...
	for (i = 0; i < 3; ++i) {
		...
		if(minc[i]<nSize)						minc[i]=nSize;
		if(maxc[i]>imDims[i]-nSize)				maxc[i]=imDims[i]-nSize;
	}
	
	float diff, val, dist[25];
	for(i=0;i<25;i++)dist[i]=9999999;
				 ...
				 int nrhs, const mxArray* prhs[])
{
	...
	cudaCheckError();
	cudaFree(d_output);		cudaFree(d_imData);
}	
